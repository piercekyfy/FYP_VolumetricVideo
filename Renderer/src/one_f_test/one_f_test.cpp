...
int main() {
	...
	SparseVoxelGrid<8> grid(0.01f, 0.02f);

	Integrate(grid, fss[0].get(), pc0.Translation);
	Integrate(grid, fss[1].get(), pc1.Translation);

	auto [points, indices] = MarchingCubes(grid, 0.0f);

	RenderedMesh r_0{};

	r_0.Update(points, indices);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (accumDelta >= FRAME && glfwGetKey(window, GLFW_KEY_F)) {
			goNext = true;
			accumDelta = 0;
		}
		...
		if (fsSource != nullptr && fsTarget != nullptr) {
			grid.Clear();
			Integrate(grid, fsSource.get(), pc0.Translation);
			Integrate(grid, fsTarget.get(), pc1.Translation);
			fsSource = nullptr;
			fsTarget = nullptr;
		...
		shader.setMatrix("mvp", GL_FALSE, glm::value_ptr(cameraState.PerspectiveMatrix() * cameraState.ViewMatrix()));
